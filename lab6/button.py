import time, math
from board import LED, A8, A5, ADC5, ADC4
import time, math
import machine




adcx = machine.ADC(machine.Pin(ADC5))
adcy = machine.ADC(machine.Pin(ADC4))
#set full-scale range
adcx.atten(machine.ADC.ATTN_0DB)

# define frequency for each tone
C3 = 131
CS3 = 139
D3 = 147
DS3 = 156
E3 = 165
F3 = 175
FS3 = 185
G3 = 196
GS3 = 208
A3 = 220
AS3 = 233
B3 = 247
C4 = 262
CS4 = 277
D4 = 294
DS4 = 311
E4 = 330
F4 = 349
FS4 = 370
G4 = 392
GS4 = 415
A4 = 440
AS4 = 466
B4 = 494
C5 = 523
CS5 = 554
D5 = 587
DS5 = 622
E5 = 659
F5 = 698
FS5 = 740
G5 = 784
GS5 = 831
A5_ = 880
AS5 = 932
B5 = 988
C6 = 1047
CS6 = 1109
D6 = 1175
DS6 = 124
E6 = 1319
F6 = 1397
FS6 = 1480
G6 = 1568
GS6 = 1661
A6 = 1760
AS6 = 1865
B6 = 1976
C7 = 2093
CS7 = 2217
D7 = 2349
DS7 = 2489
E7 = 2637
F7 = 2794
FS7 = 2960
G7 = 3136
GS7 = 3322
A7 = 3520
AS7 = 3729
B7 = 3951
C8 = 4186
CS8 = 4435
D8 = 4699
DS8 = 4978
bach = [ C4, E4, G4, C5, E5, G4, C5, E5, C4, E4, G4, C5, E5, G4, C5, E5, C4, D4, G4, D5, F5, G4, D5, F5, C4, D4, G4, D5, F5, G4, D5, F5, B3, D4, G4, D5, F5, G4, D5, F5, B3, D4, G4, D5, F5, G4, D5, F5, C4, E4, G4, C5, E5, G4, C5, E5, C4, E4, G4, C5, E5, G4, C5, E5, C4, E4, A4, E5, A5_, A4, E5, A4, C4, E4, A4, E5, A5_, A4, E5, A4, C4, D4, FS4, A4, D5, FS4, A4, D5, C4, D4, FS4, A4, D5, FS4, A4, D5, B3, D4, G4, D5, G5, G4, D5, G5, B3, D4, G4, D5, G5, G4, D5, G5, B3, C4, E4, G4, C5, E4, G4, C5, B3, C4, E4, G4, C5, E4, G4, C5, B3, C4, E4, G4, C5, E4, G4, C5, B3, C4, E4, G4, C5, E4, G4, C5, A3, C4, E4, G4, C5, E4, G4, C5, A3, C4, E4, G4, C5, E4, G4, C5, D3, A3, D4, FS4, C5, D4, FS4, C5, D3, A3, D4, FS4, C5, D4, FS4, C5, G3, B3, D4, G4, B4, D4, G4, B4, G3, B3, D4, G4, B4, D4, G4, B4 ]


buzz = machine.Pin(A5, mode=machine.Pin.OPEN_DRAIN)
pwm1 = machine.PWM(buzz, freq=500, duty=50)

def lcb(timer):
    global adcx, adcy
    x = (/255)*3.3
    y = (codey/255)*3.3
    pwm1.duty(int(x))
    pwm1.freq(int(y))
i = 0
def bcb(timer):
    global bach
    global i
    if i < len(bach):
        note = bach[i]
        pwm1.freq(note)
        i += 1

t0 = machine.Timer(2)
t0.init(period=50, mode=t0.PERIODIC, callback=lcb)
t1 = machine.Timer(3)
last_time = time.ticks_ms()

count = 0
dut = 50
p = machine.Pin(A8, mode=machine.Pin.IN, pull=machine.Pin.PULL_UP)
# pwm = machine.PWM(pin=p, freq=500, duty=50)
def handler(pin):
    global count, last_time
    t = time.ticks_ms()
    if(t-last_time) > 200 and pin.value():
        last_time = t
        t1.init(period=800, mode=t1.PERIODIC, callback=bcb)

p.irq(trigger=machine.Pin.IRQ_RISING, handler=handler)
